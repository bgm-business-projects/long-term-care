...
async function load() {
  loading.value = true
  try {
    // scope=agency 強制以登入者 organizationId 過濾，避免 admin 在此頁看到跨機構資料
    data.value = await api<any>(`/api/dispatch/dashboard?dateFrom=${dateRange.value.from}&dateTo=${dateRange.value.to}&scope=agency`)
  } finally {
    loading.value = false
  }
}
