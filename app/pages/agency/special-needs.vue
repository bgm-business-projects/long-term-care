...
async function load() {
  loading.value = true
  try {
    devices.value = await api<Device[]>('/api/dispatch/special-needs')
  } catch (err: any) {
...
async function handleSave() {
  ...
  try {
    ...
    if (editing.value) {
      await api(`/api/dispatch/special-needs/${editing.value.id}`, { method: 'PUT', body })
      toast.add({ title: '已更新', color: 'success' })
    } else {
      await api('/api/dispatch/special-needs', { method: 'POST', body })
...
async function handleDelete() {
  ...
  try {
    await api(`/api/dispatch/special-needs/${deleting.value.id}`, { method: 'DELETE' })
...
<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <h2 class="text-lg font-semibold text-highlighted">特殊需求管理</h2>
      <UButton icon="i-lucide-plus" @click="openAdd">新增特殊需求</UButton>
    </div>
    ...
    <UModal v-model:open="showModal" :title="editing ? '編輯特殊需求' : '新增特殊需求'" description=" ">
      <template #content>
        <div class="p-6 space-y-4">
          <UFormField label="名稱 *">
            <UInput v-model="form.name" placeholder="例：輕度失智、長期臥床、會自行走動" class="w-full" />
          </UFormField>
          <UFormField label="說明">
    ...
    <UModal v-model:open="showDeleteModal" title="停用特殊需求" description=" ">
      <template #content>
        <div class="p-6 space-y-4">
