...
watch(() => formData.careRecipientId, (id) => {
  ...
  if (recipient) {
    ...
    formData.originLat = recipient.lat || null
    formData.originLng = recipient.lng || null
    formData.needsWheelchair = ['wheelchair', 'bedridden'].includes(recipient.specialNeeds)
  }
})

function onServicePointSelect(val: string) {
  if (!val) return
  const sp = servicePoints.value.find((s: any) => s.id === val)
  if (sp) {
    formData.destinationAddress = sp.address || sp.name
    formData.destinationLat = sp.lat || null
    formData.destinationLng = sp.lng || null
  }
}
    ...
    <div class="space-y-5">
      ...
      <div class="space-y-2">
        ...
        <USelect
          ...
          placeholder="選擇常用服務據點..."
          class="w-full"
          @change="onServicePointSelect"
        />
        <UInput
      ...
      <div class="flex items-center gap-3 pt-2">
        ...
        <NuxtLink to="/_admin/orders" class="ml-auto">
          <UButton color="neutral" variant="ghost" icon="i-lucide-list">訂單列表</UButton>
        </NuxtLink>
      </div>
    </div>
  </div>
